...
namespace NAT.Models {
    public class TetrisGameModel : ITetrisGameModel {
        ...
        public TetrisGameModel() {
            ...
            var rnd = new Random();
            Maps[0].CurrentBlock = CreateBlock(NiceLetters.OrderBy(x => rnd.Next()).Last());
            Maps[1].CurrentBlock = CreateBlock(NiceLetters.OrderBy(x => rnd.Next()).Last());
            Maps[0].NextBlock = CreateBlock(NiceLetters.OrderBy(x => rnd.Next()).Last());
            Maps[1].NextBlock = CreateBlock(NiceLetters.OrderBy(x => rnd.Next()).Last());
        }
        ...
        private char[] NiceLetters = new char[] { 'I', 'O', 'L', 'J', 'S', 'Z', 'T' };
        ...
        public void ProccessTurn(int mapId) {
            ...
            if (MoveCurrentBlockDown(mapId) && Maps[mapId].CurrentBlockPassTurns == 0) {
                ...
                var rnd = new Random();
                var sym = NiceLetters.OrderBy(x => rnd.Next()).Last();
